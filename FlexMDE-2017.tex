...
%
\authorrunning{Dimitris Kolovos et al.} % abbreviated author list (for running head)
%
...
\begin{figure}[t!]
\centering
\includegraphics[scale=0.33]{events}
...
The \emph{ChangeEvent} class has the \emph{values} attribute which has a type of a  \emph{Collection} with generic type \emph{T}. This values attributes is used to holds one or more values (to produce model in Fig. \ref{fig:initial_chart}, \emph{e1} and \emph{e3} could be added at once as employees that \emph{e2} manages, rather than add them at a time) that are involved in an event. The generic type \emph{T} is useful for the instances of  \emph{ChangeEvent} to handle different type of values, particularly \emph{EObject} type to represent instances that \emph{EReferenceEvents} and its subclasses refer, and Java \emph{Object} type to represent literal values (e.g. Integer, String) that \emph{EAttributeEvents} and its subclasses operate. The \emph{ChangeEvent} class also has a position attribute to hold the position index of an EObject's instance or a literal when they are added into \emph{Resource}, \emph{EReference}, or \emph{EAttribute} with multiple values (Lst. \ref{lst:cbpmodel}, line 4, 7, 10, 13, 15, 18, 21). 

%When and event happens, it can involve multiple instances of \emph{EObject}. In  Lst. \ref{lst:cbpmodel} line 21, when \emph{e4} is added as an employee that \emph{e2} manages (\emph{AddToEReferenceEvent} is executed), the \emph{e4}, \emph{manages}, and \emph{e2} are set as the event's \emph{value}, \emph{eStructuralFeature} (in this case \emph{name}), and \emph{target} respectively. The \emph{eStructuralFeature} and \emph{target} attributes are defined in the \emph{EStructuralFeatureEvent} class, which are inherited by \emph{EReferenceEvent} and \emph{EAttributeEvent} and all their subclasses (Fig. \ref{fig:events}).  
...
Every time an EMF's \emph{EObject} instance is created, by default the instance is always added into a model's resource -- a default container for instances that have just created. This event is represented by the \emph{AddToResourceEvent} class. In the form of change-based XML-formatted representation, line 3, 6, 9, and 17 in Lst. \ref{lst:cbpmodel} represent this event. When the \emph{EObject} instance is deleted, or added or assigned to an EMF's \emph{EReference} object (Lst. \ref{lst:cbpmodel}, line 13, 15, 21), the event is preceded by an event that removes the instance from the resource (Lst. \ref{lst:cbpmodel}, line 12, 14, 20). This removal event is represented by the \emph{RemoveFromResourceEvent} class.

\emph{ChangeEventAdapter}, in its \emph{notifyChanged()} method, receives change notifications and filters and transforms them into appropriate change events. As an example of how notifications are filtered and transformed, Listing \ref{lst:javacode} shows how we handle \emph{Notification.UNSET} events based on the type of the changed feature i.e. an \emph{UnsetEAttributeEvent} is instantiated if the feature of the notifier is an \emph{EAttribute}, or an \emph{UnsetEReferenceEvent}  is created if the notifier is an \emph{EReference}. The transformed instances are then stored into a list of events for persistence. 
...
\begin{figure}[th]
\centering
\includegraphics[scale=0.33]{resources}
